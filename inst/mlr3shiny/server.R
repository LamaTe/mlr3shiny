# helper_files = list.files(path = "./helper", pattern="*.R")
# helper_files = paste0("helper/", helper_files)
# 
# for (i in seq_along(helper_files)) {
#   source(helper_files[i], local = TRUE)
# }
server <- function(input, output, session) {
  ...
  #notification at start-up
  # observe({
  #   showNotification(ui = "test", duration = NULL, closeButton = TRUE)
  # })
}
